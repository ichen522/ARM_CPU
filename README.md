...
Memory Dump Result *Caption: Memory dump showing a perfectly sorted array processed by Thread 1 in its isolated physical bank.*

### 2. Micro-op State Machine Execution

LDM Execution Waveform *Caption: The Control Unit intercepting an `LDMIA` instruction, stalling the pipeline, and executing sequential memory accesses.*

### 3. Pipeline Flush & Hazard Handling

Branch Flush Waveform *Caption: Correct pipeline flush execution preventing phantom instructions after a taken branch.*
